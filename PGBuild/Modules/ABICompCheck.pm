...
use PGBuild::Options;
use PGBuild::SCM;
use PGBuild::Utils qw(:DEFAULT $devnull $branch_root $steps_completed);
...
my $hooks = {
	...
	# 'need-run' => \&need_run,
	'locale-end' => \&installcheck,    # ABI comparison runs after install
...
sub setup
{
	...
	# Only proceed if this is a stable branch with git SCM
	# should be redundant - we haven't used CVS for many years
	die("Only git SCM is supported for ABICompCheck Module")
	  if ($conf->{scm} ne 'git');

	# we assume that any system that has abidw can run this module
	system("abidw --version > $devnull 2>&1");
	die("abidw not found for ABICompCheck") if $?;

	# this test had better match what's in run_build.pl
	my $using_meson =
	  $conf->{using_meson} && ($branch eq 'HEAD' || $branch ge 'REL_16_STABLE');

	# Ensure debug information is available in compilation - required for libabigail tools
	if ($using_meson)
	{
		my $meson_opts = $conf->{meson_opts} || [];
		unless (grep { $_ eq '--debug' || $_ =~ /buildtype=debug/ }
			@$meson_opts)
		{
			die("a debug build is required for ABICompCheck with meson.");
		}
	}
	else
	{
		...
		{
			die("--enable-debug is required for ABICompCheck.");
		}
	}
...
sub _get_comparison_ref_from_config
{
	...
	my @tags = run_log(qq{git -C $srcdir tag --list '$tag_pattern'})
...
sub _get_comparison_ref_from_history_file
{
	...
	open my $fh, '<', "$srcdir/.abi-compliance-history"
	  or die "Cannot open $srcdir/.abi-compliance-history: $!";
	while (my $line = <$fh>)
	{
		...
		my $exit_status =
		  system(qq{git -C $srcdir cat-file -e $line^{commit} 2>/dev/null});
		if ($exit_status != 0)
		{
			die
...
# Main function to perform ABI comparison.
#
# We're running in the locale-end step here, so the main instance is not
# running, but everything has been installed.
sub installcheck
{
	my $self = shift;
	my $locale = shift;
	...
	if (
		!(
			-f "$srcdir/.abi-compliance-history"
			|| (defined $self->{bfconf}{abi_comp_check}{tag_for_branch}
				&& exists $self->{bfconf}{abi_comp_check}{tag_for_branch}
				{ $self->{pgbranch} })
		)
	  )
	{
		...
		writelog("abi-compliance-check",
			["no .abi-compliance-history file found in $self->{pgbranch}"])
		  if $self->{pgbranch} =~ /_STABLE$/;
		return;
	}
	...
	my $latest_commit_sha = run_log(qq{git -C $srcdir rev-parse HEAD});
	die "git rev-parse HEAD failed: $?" if $?;
	chomp $latest_commit_sha;
	my $git_line = "Git HEAD: $latest_commit_sha\n";
	if ($from_source)
	{
		my $gstat = run_log("git status --porcelain -C $srcdir");
		$git_line = "Uncommitted code on top of $latest_commit_sha\n"
		  if ($gstat);
	}
	my @saveout =
	  ("Branch: $pgbranch\n", $git_line, "Changes since: $comparison_ref\n\n");
	...
	if ($rebuild_baseline)
	{
		emit "rebuilding for baseline $comparison_ref";
		...
		my $baseline_top_dir = "$abi_compare_loc/$comparison_ref";
		my $baseline_build_dir = "$baseline_top_dir/pgsql";
		my $baseline_log_dir = "$baseline_top_dir/build_logs";

		mkpath($baseline_build_dir)
		  unless -d $baseline_build_dir;
		mkpath($baseline_log_dir)
		  unless -d $baseline_log_dir;

		run_log(
			qq{git -C $srcdir archive $comparison_ref | tar xf - -C $baseline_build_dir}
		);
		die
		  "git -C $srcdir archive $comparison_ref | tar xf - -C $baseline_build_dir/pgsql failed: $?"
		  if $?;
	...
	emit("doing comparison against baseline $comparison_ref");
...
sub meson_setup
{
	...
	my $confstr = join(" ",
		"-Dauto_features=disabled", @quoted_opts, "-Dlibdir=lib",
		qq{-Dprefix="$installdir"});

	my $loc = dirname($installdir);

	# use default ninja backend on all platforms
	my @confout = run_log("meson setup $confstr $loc/pgsql.build $loc/pgsql");

	my $status = $? >> 8;

	move "$loc/pgsql.build/meson-logs/meson-log.txt",
	  "$loc/pgsql.build/meson-logs/setup.log";

	my $log = PGBuild::Log->new($baseline . "setup");
	foreach my $logfile (
		"$loc/pgsql.build/meson-logs/setup.log",
		"$loc/pgsql.build/src/include/pg_config.h"
	  )
	{
...
sub configure
{
	...
	if ($self->{using_meson})
	{
		$self->meson_setup("$abi_compare_loc/$baseline/inst", $baseline);
		return;
	}
...
sub make
{
	...
	if ($self->{using_meson})
	{
		...
		@makeout = run_log("meson compile -C $pgsql.build --verbose $jflag");
		move "$pgsql.build/meson-logs/meson-log.txt",
		  "$pgsql.build/meson-logs/compile.log";
		if (-s "$pgsql.build/meson-logs/compile.log")
		{
			my $log = PGBuild::Log->new("compile");
			$log->add_log("$pgsql.build/meson-logs/compile.log");
...
sub make_install
{
	...
	if ($self->{using_meson})
	{
		@makeout = run_log("meson install -C $pgsql.build ");
		move "$pgsql.build/meson-logs/meson-log.txt",
		  "$pgsql.build/meson-logs/install.log";
		my $log = PGBuild::Log->new("install");
		if (-s "$pgsql.build/meson-logs/install.log")
		{
			$log->add_file("$pgsql.build/meson-logs/install.log");
			push(@makeout, $log->log_string);
		}
	}
	else
...
sub _generate_abidw_xml
{
	...
	while (my ($target_name, $rel_path) = each %{$binaries_rel_path})
	{
		...
		{
			emit "FAILURE - $target_name not found at $input_path";
		}
	}
...
sub cleanup
{
	...
	if (!$keepall)
	{
		...
		return
		  unless $current_baseline;  # this could happen only in some worst case
		...
		rmtree("$abi_compare_loc/$current_baseline/pgsql.build")
		  if -d "$abi_compare_loc/$current_baseline/pgsql.build";
